...
import LoginScreen, { getLoggedInUser, logout } from './components/LoginScreen';
...
export default function App() {
  ...
  const mentionCount = useMemo(() =>
    tasks.filter(t => (t.mentioned_ids || []).includes(currentUser?.id)).length,
  [tasks, currentUser]);
  ...
  function handleNewTask() {
    createTask({ title: 'Untitled task', status: 'Open', priority: 'Normal', owner_id: currentUser?.id || null, assignee_ids: currentUser ? [currentUser.id] : [] })
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Inter, sans-serif', background: T.bg, overflow: 'hidden' }}>
      <Sidebar
        view={view} setView={setView} team={team}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {!noTopBar && (
          <TopBar
            view={view} search={search} setSearch={setSearch}
            onNewTask={handleNewTask} filters={filters} setFilters={setFilters} tasks={tasks}
          />
        )}
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          {loading ? (
            ...
            <>
              {view === 'dashboard' && (
                <DashboardView tasks={tasks} team={team} onOpenTask={setSelectedTask} currentUser={currentUser} />
              )}
              {view === 'kanban' && (
              ...
              {view === 'mentions' && (
                <MentionsView tasks={tasks} team={team} onOpenTask={setSelectedTask} onUpdate={updateTask} currentUserId={currentUser?.id} />
              )}
            </>
          )}
        </div>
      ...
      {selectedTask && tasks.find(t => t.id === selectedTask.id) && (
        <TaskModal
          ...
          onCreateSubtask={data => createTask(data)}
          onAddComment={addComment} getComments={getComments}
        ...
        <div onClick={() => setShowAddMember(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(45,27,78,0.25)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: T.card, borderRadius: T.radius, padding: 28, width: 340, boxShadow: T.shadow }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <input value={newMember.name} onChange={e => setNewMember(m => ({ ...m, name: e.target.value }))} placeholder="Full name" style={fieldStyle} />
              <input value={newMember.email} onChange={e => setNewMember(m => ({ ...m, email: e.target.value }))} placeholder="Email address" style={fieldStyle} />
              <div>
